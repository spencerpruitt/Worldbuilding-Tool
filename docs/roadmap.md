...
| **Re-platform foundation spike** (React + build, one real surface end-to-end, `.map` intact) | `feat/replatform-foundation` | from `prds/backlog/replatform-program.md` step 2 (**needs its own PRD + an ADR for React/build tooling choices**) | HITL (visual) | Wave 1 (AR-1 serialization boundary) | ⬜ not started |
